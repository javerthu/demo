...
class OrderCommitView1(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            for sku_id in sku_ids:
                ...
                try:
                    ...
                    # select_for_update() 相当获得了锁, sql: select * from df_goods_sku where id=sku_id for update;悲观锁谁先拿到谁先处理
        ...
        # 返回应答
        return JsonResponse({'res': 5, 'message': '订单创建成功javert'})
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        total_pay = order.total_price + order.transit_price #模型models里是Decimal类型，在这拿到的也是一样的类型(不能序列化，需要str）
        order_string = alipay.api_alipay_trade_page_pay(
            out_trade_no=order_id,  # 订单id
            total_amount=str(total_pay),  # 支付总金额
            subject='天天生鲜%s 用户' % user.username,  #订单标题
            return_url=None,
...
class CheckPayView(View):
    ...
    def post(self, request):
        ...
        while True:
            ...
            #正式场景需要验证签名信息，开发过程需要注释掉源码中的if not语句。

            #返回字典的格式
            # response = {
            #     "trade_no": "2017032121001004070200176844", #支付宝交易号
            #     "code": "10000", #接口调用结果返回码
